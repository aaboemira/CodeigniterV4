...
class elFinderVolumeMySQL extends elFinderVolumeDriver {
	...
	protected function init() {
		...
		if ($builder->connect_error || mysqli_connect_error()) {
			return false;
		}
		
		$builder->set_charset('utf8');
	...
	protected function make($path, $name, $mime) {
		...
		$sql = sprintf($sql, $this->tbf, $path, $builder->real_escape_string($name), time(), $mime, $this->defaults['read'], $this->defaults['write'], $this->defaults['locked'], $this->defaults['hidden']);
		// echo $sql;
		return $this->query($sql) && $builder->affected_rows > 0;
	}
	...
	protected function loadFilePath($path) {
		...
		return $builder->real_escape_string($realPath);
	}
		...
		if ($mimes) {
			...
			foreach($mimes as $mime) {
				if (strpos($mime, '/') === false) {
					$whrs[] = sprintf('f.mime LIKE \'%s/%%\'', $builder->real_escape_string($mime));
				} else {
					$whrs[] = sprintf('f.mime = \'%s\'', $builder->real_escape_string($mime));
				}
			}
			$whr = join(' OR ', $whrs);
		} else {
			$whr = sprintf('f.name LIKE \'%%%s%%\'', $builder->real_escape_string($q));
		}
		if ($dirs) {
	...
	protected function _joinPath($dir, $name) {
		$sql = 'SELECT id FROM '.$this->tbf.' WHERE parent_id=\''.$dir.'\' AND name=\''.$builder->real_escape_string($name).'\'';
	...
	protected function _copy($source, $targetDir, $name) {
		...
		$sql = $id > 0
			...
			: sprintf('INSERT INTO %s (parent_id, name, content, size, mtime, mime, width, height, `read`, `write`, `locked`, `hidden`) SELECT %d, \'%s\', content, size, %d, mime, width, height, `read`, `write`, `locked`, `hidden` FROM %s WHERE id=%d', $this->tbf, $targetDir, $builder->real_escape_string($name), time(), $this->tbf, $source);
	...
	protected function _move($source, $targetDir, $name) {
		...
		$sql = sprintf($sql, $this->tbf, $targetDir, $builder->real_escape_string($name), $source);
		return $this->query($sql) && $builder->affected_rows > 0 ? $source : false;
	}
	...
	protected function _unlink($path) {
		return $this->query(sprintf('DELETE FROM %s WHERE id=%d AND mime!=\'directory\' LIMIT 1', $this->tbf, $path)) && $builder->affected_rows;
	}
	...
	protected function _save($fp, $dir, $name, $stat) {
		...
		if (($tmpfile = tempnam($this->tmpPath, $this->id))) {
			if (($trgfp = fopen($tmpfile, 'wb')) == false) {
				...
				$sql = sprintf($sql, $this->tbf, $dir, $builder->real_escape_string($name), $this->loadFilePath($tmpfile), $size, time(), $mime, $w, $h);

				$res = $this->query($sql);
				unlink($tmpfile);
				
				if ($res) {
					return $id > 0 ? $id : $builder->insert_id;
				}
			}
		}
		...
		$sql = sprintf($sql, $this->tbf, $dir, $builder->real_escape_string($name), $builder->real_escape_string($content), $size, time(), $mime, $w, $h);
	...
	protected function _filePutContents($path, $content) {
		return $this->query(sprintf('UPDATE %s SET content=\'%s\', size=%d, mtime=%d WHERE id=%d LIMIT 1', $this->tbf, $builder->real_escape_string($content), strlen($content), time(), $path));
	}
