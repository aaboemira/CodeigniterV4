...
class elFinderVolumeDropbox extends elFinderVolumeDriver {
	...
	public function netunmount($netVolumes, $key) {
		...
		if ($count === 1) {
			$builder->exec('drop table '.$this->DB_TableName);
	...
	private function checkDB() {
		...
		if (! $res) {
			try {
				$builder->exec('CREATE TABLE '.$this->DB_TableName.'(path text, fname text, dat blob, isdir integer);');
				$builder->exec('CREATE UNIQUE INDEX nameidx ON '.$this->DB_TableName.'(path, fname)');
				$builder->exec('CREATE INDEX isdiridx ON '.$this->DB_TableName.'(isdir)');
			} catch (PDOException $e) {
	...
	private function getDBdat($path) {
		if ($res = $this->query('select dat from '.$this->DB_TableName.' where path='.$builder->quote(strtolower($this->_dirname($path))).' and fname='.$builder->quote(strtolower($this->_basename($path))).' limit 1')) {
	...
	private function updateDBdat($path, $dat) {
		return $this->query('update '.$this->DB_TableName.' set dat='.$builder->quote(serialize($dat))
				. ', isdir=' . ($dat['is_dir']? 1 : 0)
				. ' where path='.$builder->quote(strtolower($this->_dirname($path))).' and fname='.$builder->quote(strtolower($this->_basename($path))));
	}
	/*********************************************************************/
	/*                               FS API                              */
	/*********************************************************************/
		...
		try {
			...
			do {
				...
				if (! empty($_info['reset'])) {
					$builder->exec('TRUNCATE table '.$this->DB_TableName);
					$builder->exec('insert into '.$this->DB_TableName.' values(\'\', \'\', \''.serialize(array('cursor' => '', 'mtime' => 0)).'\', 0);');
					$builder->exec('insert into '.$this->DB_TableName.' values(\'/\', \'\', \''.serialize(array(
						'path'      => '/',
						'is_dir'    => 1,
						'mime_type' => '',
						'bytes'     => 0
				...
				foreach($_info['entries'] as $entry) {
					...
					$path = $builder->quote($pkey);
					$fname = $builder->quote(strtolower($this->_basename($key)));
					$where = 'where path='.$path.' and fname='.$fname;
					
					if (empty($entry[1])) {
						$ptimes[$pkey] = isset($ptimes[$pkey])? max(array($now, $ptimes[$pkey])) : $now;
						$builder->exec('delete from '.$this->DB_TableName.' '.$where);
						! $delete && $delete = true;
						continue;
					}
					...
					if (! $reset && $this->query($sql)) {
						$builder->exec('update '.$this->DB_TableName.' set dat='.$builder->quote(serialize($entry[1])).', isdir='.($entry[1]['is_dir']? 1 : 0).' ' .$where);
					} else {
						$builder->exec('insert into '.$this->DB_TableName.' values ('.$path.', '.$fname.', '.$builder->quote(serialize($entry[1])).', '.(int)$entry[1]['is_dir'].')');
					}
				}
			...
			foreach ($ptimes as $_p => $_t) {
				if ($praw = $this->getDBdat($_p)) {
					...
					if ($_update) {
						$pwhere = 'where path='.$builder->quote(strtolower($this->_dirname($_p))).' and fname='.$builder->quote(strtolower($this->_basename($_p)));
						$builder->exec('update '.$this->DB_TableName.' set dat='.$builder->quote(serialize($praw)).' '.$pwhere);
					}
				}
			}
			
			$builder->exec('update '.$this->DB_TableName.' set dat='.$builder->quote(serialize(array('cursor'=>$cursor, 'mtime'=>$_SERVER['REQUEST_TIME']))).' where path=\'\' and fname=\'\'');
			if (! $builder->commit()) {
				$e = $builder->errorInfo();
				return $e[2];
			}
	...
	protected function parseRaw($raw) {
		...
		$stat['dirs'] = 0;
		if ($raw['is_dir']) {
			$stat['dirs'] = (int)(bool)$this->query('select path from '.$this->DB_TableName.' where isdir=1 and path='.$builder->quote(strtolower($raw['path'])));
		}
	...
	protected function cacheDir($path) {
		...
		$res = $this->query('select dat from '.$this->DB_TableName.' where path='.$builder->quote(strtolower($path)));
	...
	**/
	protected function doSearch($path, $q, $mimes) {
		$result = array();
		$sth = $builder->prepare('select dat from '.$this->DB_TableName.' WHERE path LIKE ? AND fname LIKE ?');
