    ...
    if (!empty($orders)) {
        ...
        <div style="margin-bottom:10px;">
            <select class="selectpicker changeOrder">
                <option <?= isset($_GET['order_by']) && $_GET['order_by'] == 'id' ? 'selected' : '' ?> value="id">Order
                    by new
                </option>
                <option <?= (isset($_GET['order_by']) && $_GET['order_by'] == 'processed') || !isset($_GET['order_by']) ? 'selected' : '' ?>
                        value="processed">Order by not processed
                </option>
            </select>
        </div>
                        ...
                        <td class="relative" id="order_id-id-<?= $tr['order_id'] ?>">
                            ...
                            <?php if ($tr['viewed'] == 0) { ?>
                                <div id="new-order-alert-<?= $tr['id'] ?>">
                                    <img src="<?= base_url('assets/imgs/new-blinking.gif') ?>" style="width:100px;"
                                         alt="blinking">
                                </div>
                            <?php } ?>
                                ...
                                <div style="margin-bottom: 4px;">
                                    <form id="sendMailForm" method="post">
                                        <div class="input-group" style="margin-bottom:10px">
                                            <input placeholder="Shipping Number" type="text" class="form-control"
                                        ...
                                        <input type="hidden" id="full_name" name="full_name"
                                               value="<?= $tr['first_name'] . ' ' . $tr['last_name'] ?>">
                                        <input type="hidden" id="country" name="country" value="<?= $tr['country'] ?>">
                                        <input type="hidden" id="email" name="email" value="<?= $tr['email'] ?>">
                                        <input type="hidden" id="housenr" name="addr_1"
                                               value="<?= $tr['street'] . ' ' . $tr['housenr'] ?>">
                                        <input type="hidden" id="post_code" name="addr_2"
                                               value="<?= $tr['post_code'] . ' ' . $tr['city'] ?>">
                                        <input type="hidden" id="company" name="company" value="<?= $tr['company'] ?>">
                                        <input type="hidden" id="shipping_type" name="shipping_type"
                                               value="<?= $tr['shipping_type'] ?>">
                                        <input type="hidden" id="payment_type" name="payment_type"
                                               value="<?= $tr['payment_type'] ?>">
                                        <input type="hidden" id="order_id" name="order_id"
                                               value="<?= $tr['order_id'] ?>">
                                        <input type="hidden" id="discount" name="discount"
                                               value="<?= $tr['discount'] ?>">
                                        <input type="hidden" id="arr_products" name="products"
                                               value="<?= htmlentities($tr['products']) ?>">
                                        <input type="hidden" id="order_date" name="order_date"
                                               value="<?= $tr['date'] ?>">
                                        <input type="hidden" id="shipping_price" name="shipping_price"
                                               value="<?= $tr['shipping_price'] ?>">


                                        <button type="submit" class="btn btn-warning btn-xs">Sende Bestätigungsmail
                                        </button>
                                    </form>
                                </div>
                            </div>
                                ...
                                <table class="table more-info-purchase">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <?php if ($tr['referrer'] != 'Direct') { ?>
                                                <a target="_blank" href="<?= $tr['referrer'] ?>"
                                                   class="orders-referral">
                                                    <?= $tr['referrer'] ?>
                                                </a>
                                            <?php } else { ?>
                                    ...
                                    <tr>
                                        <td colspan="2">
                                            ...
                                            <div style="padding-top:10px; font-size:16px;">Total amount of order mwst
                                                19%:
                ...
                <div class="panel-body">
                    ...
                    <form method="POST" action="">
                        ...
                        <input <?= $cashondelivery_visibility == 1 ? 'checked' : '' ?> data-toggle="toggle"
                                                                                       data-for-field="cashondelivery_visibility"
                                                                                       class="toggle-changer"
                                                                                       type="checkbox">
                    ...
                    <table class="table table-bordered">
                        ...
                        <tr>
                            <td colspan="2"><input type="text"
                                                   value="<?= $bank_account != null ? $bank_account['bank'] : '' ?>"
                                                   name="bank"
                                                   class="form-control" placeholder="Example: First Investment Bank">
                            </td>
                        </tr>
                        </tbody>
                    </table>
