...
<div style="margin-bottom:10px;">
    <select class="selectpicker changeOrder">
        ...
        <option
            <?= (isset($_GET['order_by']) && $_GET['order_by'] == 'processed') || !isset($_GET['order_by']) ? 'selected' : '' ?>
            ...
            <tr>
                ...
                <td class="hidden" id="order-id-<?= $tr['order_id'] ?>">
                    <div class="container">
                        ...
                        <div> <?= $tr['email'] ?></div>
                        <div> <?= 'SHND' . $tr['order_id'] ?></div>
                        <div> <?= $tr['first_name'] . ' ' . $tr['last_name'] ?></div>
                        <div> <?= $tr['company'] ?></div>
                        <div> <?= $tr['street'] ?> </div>
                        <div> <?= $tr['housenr']?> </div>
                        <div> <?= $tr['post_code'] ?> </div>
                        <div> <?= $tr['city'] ?> </div>
                        <div><?= $tr['country'] ?></div>
                    </div>
                    ...
                    <div class="container">
                        ...
                        <div> <?= $tr['first_name'] . ' ' . $tr['last_name'] ?></div>
                        <div> <?= $tr['company'] ?></div>
                        <div> <?= $tr['street'] . ' ' . $tr['housenr']?> </div>
                        <div> <?= $tr['post_code'] . ' ' . $tr['city'] ?> </div>
                        <div><?= $tr['country'] ?></div>
                    </div>

                    <div class="container">
                        <div style="margin-bottom:4px;"><a href="javascript:void(0);"
                                onclick="sendBestellbestaetigung()" class="btn btn-warning btn-xs">Sende
                                Bestätigungsmail</a></div>
                    </div>
                    ...
                    <div class="table-responsive">
                        <table class="table more-info-purchase">
                            <tbody>
                                ...
                                <tr>
                                    <td><b>Discount</b></td>
                                    <td><?= $tr['discount_type'] == 'float' ? '-' . $tr['discount_amount'] : '-' . $tr['discount_amount'] . '%' ?>
                                    </td>
                                </tr>
                                ...
                                <td><b>Shipping Type</b></td>
                                    <td><?= $tr['shipping_type'] ?></td>
                                </tr>
                                <tr>
                                    <td><b>Shipping Price</b></td>
                                    <td><?= $tr['shipping_price'] ?></td>
                                </tr>
                                <tr>
...
?>
                                        <div style="word-break: break-all;">
                                            ...
                                            <a data-toggle="tooltip" data-placement="top" title="Click to preview"
                                                ...
                                                <div
                                                    ...
                                                    <b>Price:
                                                        <?= $product['product_info']['price'].' '.config('config')->currency ?></b>
                                                </div>
                                            </a>
                                            <div class="">
                                                <b>Vendor:</b>
                                        ...
                                        <div style="padding-top:10px; font-size:16px;">Total amount of products:
                                            <?= $total_amount.' '.config('config')->currency ?></div>
                                        <hr>
                                        <?php }
...
<div style="padding-top:10px; font-size:16px;">Total amount of order netto:
                                            <?= $total_amount_order_netto.' '.config('config')->currency ?></div>
<div style="padding-top:10px; font-size:16px;">Total amount of order mwst 19%:
                                            <?= $total_order_mwst19.' '.config('config')->currency ?></div>
<div style="padding-top:10px; font-size:16px;">Total amount of order brutto:
                                            <?= $total_amount_order.' '.config('config')->currency ?></div>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
    ...
    <div class="col-sm-4">
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                <?php if (session('cashondelivery_visibility')) { ?>
                <div class="alert alert-info"><?= session('cashondelivery_visibility') ?></div>
                <?php } ?>
...
<div class="row">
    <div class="col-sm-6">
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                <?php if (session('paypal_sandbox')) { ?>
                <div class="alert alert-info"><?= session('paypal_sandbox') ?></div>
                <?php } ?>
                <form method="POST" action="">
    ...
    <div class="col-sm-6">
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                <?php if (session('paypal_email')) { ?>
                <div class="alert alert-info"><?= session('paypal_email') ?></div>
                <?php } ?>
                <form method="POST" action="">
...
<div class="row">
    <div class="col-sm-6">
        <?php if (session('bank_account')) { ?>
        <div class="alert alert-info"><?= session('bank_account') ?></div>
        <?php } ?>
        <form method="POST" action="">
            <div class="table-responsive">
